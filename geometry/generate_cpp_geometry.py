...
inst = d["instances"]
lines.append(
    f"inline constexpr std::array<double,{len(inst)}> rackU = {{{', '.join(str(x['rack_u']) for x in inst)}}};"
)
pieces = [
    p | {"layer": L["id"], "offset": L["channel_offset"]}
    for L in d["layers"]
    for p in L["pieces"]
]
...
for p in pieces:
    ...
    lines.append(
        f" Piece{{{p['layer']},{p['bar_id']},{p['offset']},{p['center_index']},{str(p['sensitive']).lower()}, {{{vs}}}}},"
    )
lines += ["}};", "}"]
