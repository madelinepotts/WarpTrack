...
static G4TessellatedSolid *makePrism(const Piece &p, double base, double height,
                                     double length) {
  ...
  for (int i = 0; i < 3; ++i) {
    int j = (i + 1) % 3;
    s->AddFacet(new G4QuadrangularFacet(a[i], a[j], b[j], b[i], ABSOLUTE));
  }
...
G4VPhysicalVolume *DetectorConstruction::Construct() {
  ...
  auto *air = n->FindOrBuildMaterial("G4_AIR");
  auto *scint = n->FindOrBuildMaterial("G4_PLASTIC_SC_VINYLTOLUENE");
  const double width = widthMM * mm, depth = depthMM * mm,
               hh = hodoscopeHeightMM * mm, lh = hh / 2;
  const double bottomBase = 2 * depth / 15.0, topBase = 2 * width / 8.0;
  auto *ws = new G4Box("World", 1.5 * m, 1.5 * m, 2 * m);
  auto *wlv = new G4LogicalVolume(ws, air, "WorldLV");
  auto *wpv =
      new G4PVPlacement(nullptr, {}, wlv, "WorldPV", nullptr, false, 0, true);
  ...
  for (size_t h = 0; h < rackU.size(); ++h) {
    ...
    for (const auto &p : pieces) {
      ...
      const double base = bottom ? bottomBase : topBase;
      const double span = bottom ? depth : width;
      const double length = bottom ? width : depth;
      const double pitch = base / 2;
      const double u = -span / 2 + base / 2 + p.centerIndex * pitch;
      const double z = cz + (bottom ? -lh / 2 : lh / 2);
      auto *solid = makePrism(p, base, lh, length);
      std::string name = (bottom ? "Bottom" : "Top") + std::string("BarLV_") +
                         std::to_string(p.bar);
      auto *lv = new G4LogicalVolume(solid, scint, name);
      if (p.sensitive)
        scintillatorLVs_.push_back(lv);
      G4ThreeVector pos =
          bottom ? G4ThreeVector(0, u, z) : G4ThreeVector(u, 0, z);
      const int channel =
          int(h) * channelsPerHodoscope + p.channelOffset + p.bar;
      new G4PVPlacement(nullptr, pos, lv, bottom ? "BottomBarPV" : "TopBarPV",
